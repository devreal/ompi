 ...
 * device memory, not hipMallocManaged: it's touched by the host and the
 * kernel on every single reduction call, and that back-and-forth drove
 * constant page migration faults under managed memory. Instead the host
 * stages each command in a registered (page-locked) mirror, cq->host_cmd,
 * and moves it to/from the device slot with explicit hipMemcpyAsync on a
 ...
 * resets status back to 0 itself before exiting (see op_rocm_kernels.cpp),
 ...
 *                  exit.  The cmd_queue's HIP stream and device/host memory
 ...
 * reduction call, and to signal shutdown), and two private HIP streams --
 * one for the persistent kernel, one for host<->device transfers.
 * -------------------------------------------------------------------------- */
ompi_op_gpu_cmd_queue_t *
                            ...
                            struct ompi_datatype_t *dtype)
{
     ...
     * reset here. See ompi_op_rocm_session_reduce() for the actual launch. */
                             ...
                             void *dst, size_t count)
{
    ...
    if (!session->started) {
        ...
        ompi_op_rocm_launcher_fn_t launcher = (ompi_op_rocm_launcher_fn_t) session->launcher;
        launcher(dev_cmd, cq->stream, cq->super.dev_id);
        hipError_t err = hipGetLastError();
        if (hipSuccess != err) {
            /* session_begin already validated that a kernel exists for this
             * (op, dtype), so a failure here is a genuine HIP runtime/driver
            ...
            opal_output(0, "op/rocm: persistent kernel launch failed on device %d: %s",
                       cq->super.dev_id, hipGetErrorString(err));
            abort();
        }
...
ompi_op_rocm_session_stop(ompi_op_gpu_session_t *session)
{
    ...
    host_cmd->status = -1;
    hipMemcpyAsync((void *) &dev_cmd->status, (const void *) &host_cmd->status,
                   sizeof(host_cmd->status), hipMemcpyHostToDevice, cq->ctrl_stream);
